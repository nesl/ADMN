...
from PickleDataset import PickleDataset, transform_mask, transform_noise, transform_finite_noise, transform_set_noise, transform_discrete_noise
...
def main(args):
    ...
    model = GTDM_Early(args.adapter_hidden_dim, valid_mods=args.valid_mods, valid_nodes = args.valid_nodes, drop_layers_img=args.drop_layers_img, drop_layers_depth=args.drop_layers_depth) # Pass valid mods, nodes, and also hidden layer size
    # Load model weights
    ...
    testset = PickleDataset(args.cache_dir + '/test/', valid_mods = args.valid_mods, valid_nodes=args.valid_nodes)
    ...
    for batch in tqdm(test_dataloader, desc = 'Computing test loss', leave=False):
        with torch.no_grad():
            ...
            data, _ = transform_finite_noise(data, args.batch_size, img_std_max=3, depth_std_max=0, mmWave_std_max=0.8)
