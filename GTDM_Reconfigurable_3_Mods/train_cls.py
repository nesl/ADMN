...
from PickleDataset import PickleDataset, transform_inputs, transform_noise, transform_set_noise, transform_finite_noise, transform_mask
...
def main(args):
    ...
    model = GTDM_Controller(args.adapter_hidden_dim, valid_mods=args.valid_mods, valid_nodes = args.valid_nodes, total_layers=args.total_layers)
    model.to(device)
    print(model.load_state_dict(torch.load('./logs/AWGN_Img3_Depth75/last.pt'), strict=False))
    ...
    for epoch in trange(args.num_epochs, desc="Training"):
        ...
        for batch in train_dataloader:
            ...
            data = transform_finite_noise(data, args.batch_size)
