...
from PickleDataset import transform_noise
...
def main(args):
    ...
    model = GTDM_Early(args.adapter_hidden_dim, valid_mods=args.valid_mods, valid_nodes = args.valid_nodes, layerdrop=0.0, vision_vit_layers=args.vision_vit_layers, depth_vit_layers=args.depth_vit_layers, from_scratch=args.from_scratch)
    model.to(device)
    ...
    for epoch in trange(args.num_epochs, desc="Training"):
        ...
        for batch in train_dataloader:
            ...
            # Do not add noise to the first 5 epochs
            if epoch < 5:
                data, _ = transform_noise(data, args.batch_size, img_std_max = 0, depth_std_max = 0)
            else:
