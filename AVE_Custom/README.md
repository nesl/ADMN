## Change File Paths:
Under `configs.py`, change the `cached-root` to point to the absolute path of the `AVE_Dataset_Cached` directory. Then, change the `pretrained_path_img` and `pretrained_path_audio` to point to the location of the `MAE_Dropout_FT_Dropout.pth` and `AudioMAE_Dropout_FT_Dropout.pth`, respectively.
