...
def cache_data(cached_root):
    if not os.path.isdir(cached_root + '/train/'):
        print("Caching Train")
        os.mkdir(cached_root + '/train/')
        trainset = AVE_Dataset(dataset_root = '/mnt/ssd_8t/jason/AVE_Dataset/', dataset_txt_file='trainSet.txt')
        ctr = 0
    ...
    if not os.path.isdir(cached_root + '/val/'):
        ...
        trainset = AVE_Dataset(dataset_root = '/mnt/ssd_8t/jason/AVE_Dataset/', dataset_txt_file='valSet.txt')
        ctr = 0
        for item in tqdm(trainset):
    ...
    if not os.path.isdir(cached_root + '/test/'):
        ...
        trainset = AVE_Dataset(dataset_root = '/mnt/ssd_8t/jason/AVE_Dataset/', dataset_txt_file='testSet.txt')
        ctr = 0
        for item in tqdm(trainset):
