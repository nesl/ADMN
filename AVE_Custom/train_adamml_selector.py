...
from models.GTDM_Model import AdaMML_Model_All, AdaMML_Modality_Selector
...
def get_args_parser():
    ...
    parser.add_argument("--base_root", type=str, default = '/mnt/ssd2/data/AVE_Dataset/', help="Base dataset root")
    ...
    parser.add_argument("--audio_ckp_path", type=str, default='logs/AdaMML_Subnet_Dep_Test/last.pt', help="path of the pretrained depth recognition model")
...
def main(args):
    ...
    #cache_data(cached_root = '/mnt/hdd1/yuyang/data/AVE_Dataset_Cached/')
    #PickleDataset inherits from a Pytorch Dataset, creates train and val datasets
    trainset = PickleDataset(data_root = '/mnt/ssd2/data/AVE_Dataset_Cached/', type='val')
    valset = PickleDataset(data_root = '/mnt/ssd2/data/AVE_Dataset_Cached/', type='val')
    ...
    print(model.audio.load_state_dict(torch.load(args.audio_ckp_path, weights_only=False), strict=True))
    ...
    for epoch in trange(args.num_epochs, desc="Training"):
        ...
        for batch in train_dataloader:
            ...
            batch_results, selector = model(batch)
            ...
            with torch.no_grad():
                ...
                print("Batch Num: ", batch_num, 'Train Loss', train_loss.detach().cpu().item(), 'Selector', selector, 'Noise Label', noise_label[0])
                print('\n')
        ...
        with torch.no_grad():
            ...
            for batch in val_dataloader:
                ...
                _, _, label = batch
                gt_labels.append(label.cpu())
                batch_results, selector = model(batch) #Dictionary
        ...
        with open( './logs/' + dt_string + '/log.txt', 'a') as handle:
            print('Epoch ' + str(epoch) + ' | Train loss ' + str(epoch_train_loss) +  ' | Train Acc ' + str(train_acc)  + ' | Val Accuracy ' + str(val_acc)
                  , file=handle)
