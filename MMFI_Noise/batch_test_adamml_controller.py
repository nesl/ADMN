...
from models.MMFI_Model import AdaMML_Model_All
...
def main(args):
    ...
    model = AdaMML_Model_All(adapter_hidden_dim=args.adapter_hidden_dim, total_layers=args.total_layers) # Pass valid mods, nodes, and also hidden layer size
    # Load model weights
    ...
    for batch in tqdm(test_dataloader, desc = 'Computing test loss', leave=False):
        with torch.no_grad():
            ...
            results, _ = model(batch) # Evaluate on test data
