'''
Run finetune_layers.py after we train a multimodal network through train.py
Specify the layers we want to drop, and then freeze the backbones
This serves as a point of comparison against the dynamic counterpart, since it undergoes additional training with a specific layer budget
...
from models.MMFI_Model import GTDM_Early
import argparse
import random
...
def get_args_parser():
    ...
    parser.add('--drop_layers', type=int, nargs="+", help='List of layers to DROP')
    parser.add('--checkpoint', type=str, help="The multimodal model we want to finetune on specific layer config")
...
def main(args):
    ...
    model = GTDM_Early(args.adapter_hidden_dim, valid_mods=args.valid_mods, valid_nodes = args.valid_nodes, drop_layers=args.drop_layers)
