...
def main(args):
    ...
    dt_string = "AE_Controller_" + 'Layer_' + str(args.total_layers) + '_Seed_' + str(args.seedVal)
    ...
    for epoch in trange(args.num_epochs, desc="Training"):
        ...
        for batch in train_dataloader:
            ...
            gt_img_blur = data['blur', 'blur']
            gt_depth_blur = torch.zeros_like(gt_img_blur)
            data = transform_data(data)
            # Perform forward pass
            batch_results, pred_img_blur, pred_depth_blur = model(data, controller_temperature = args.temp, discretization_method=args.discretization_method) #Dictionary
            print(gt_img_blur[0], gt_depth_blur[0], pred_img_blur[0], pred_depth_blur[0])

            for i in range(len(gt_img_blur)):
                if gt_img_blur[i] == 51:
                    gt_img_blur[i] = 1
                if gt_img_blur[i] == 201:
                    gt_img_blur[i] = 2

                if gt_depth_blur[i] == 51:
                    gt_depth_blur[i] = 1
                if gt_depth_blur[i] == 201:
                    gt_depth_blur[i] = 2
            ...
            for key in batch_results.keys():
                for i in range(len(batch_results[key]['dist'])):
                    ...
                    # No longer consider NLL loss since we can't change the covariances??
                    train_loss += 0.05 * loss_mse # Accumulate all the losses into the batch loss
                    with torch.no_grad():
            ...
            # train_loss += ce_loss(pred_img_blur, gt_img_blur.cuda()) + ce_loss(pred_depth_blur, gt_depth_blur.cuda())
            with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            for batch in val_dataloader:
                ...
                # Perform forward pass
                batch_results, pred_img_blur, pred_depth_blur = model(data, controller_temperature=1,  discretization_method=args.discretization_method) #Dictionary
                print(pred_img_blur[0], pred_depth_blur[0])
                # key is still ('modality', 'node') with a distribution estimated by the model
                for key in batch_results.keys():
                    for i in range(len(batch_results[key]['dist'])):     
                        loss_mse = mseloss(torch.squeeze(batch_results[key]['dist'][i].mean), torch.squeeze(gt_pos[i][:, [0, 2]]))
