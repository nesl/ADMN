## Change File Paths:
Under `configs/config.yaml`, change the cache_dir to point to the location of your `Original` folder. *Make sure to include a trailing slash*. Additionally, in `models/GTDM_Model.py`, change lines 41 and 54 (the torch.load function) to load the correct `MAE_Dropout_FT_Dropout.pth` file that you downloaded.
